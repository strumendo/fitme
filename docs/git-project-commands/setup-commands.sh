#!/usr/bin/env bash
# Bootstraps the GitHub Project board described in project-structure.md.
# Idempotent for labels (--force); issues/milestones/project are NOT idempotent —
# re-running will create duplicates. Use the rollback section in setup-commands.md
# if you need a clean slate.
#
# Run from anywhere; the script does not depend on the working directory.

set -euo pipefail

REPO="strumendo/fitme"
...
gh label create "type:epic" --color B60205 --description "Tracks a whole phase" --force --repo "$REPO"
gh label create "type:task" --color 0E8A16 --description "Concrete, mergeable unit of work" --force --repo "$REPO"

gh label create "phase:1" --color 1D76DB --description "Phase 1 — Persistence" --force --repo "$REPO"
gh label create "phase:2" --color 5319E7 --description "Phase 2 — Time series" --force --repo "$REPO"
gh label create "phase:3" --color D93F0B --description "Phase 3 — Garmin metrics" --force --repo "$REPO"
gh label create "phase:4" --color FBCA04 --description "Phase 4 — Manual inputs" --force --repo "$REPO"

gh label create "area:db"     --color C5DEF5 --description "Schema, migrations, SQLite" --force --repo "$REPO"
gh label create "area:ingest" --color BFD4F2 --description "Garmin → DB ingestion" --force --repo "$REPO"
gh label create "area:ui"     --color D4C5F9 --description "Streamlit pages and forms" --force --repo "$REPO"
gh label create "area:garmin" --color F9D0C4 --description "fitme.garmin / Garmin API" --force --repo "$REPO"
gh label create "area:docs"   --color FEF2C0 --description "CLAUDE.md, plans, READMEs" --force --repo "$REPO"

# ----- 2. Milestones -------------------------------------------------------

gh api "repos/$REPO/milestones" -f title="Phase 1 — Persistence"     -f description="SQLite local DB + ingest CLI. See docs/plans/01-persistence.md."
gh api "repos/$REPO/milestones" -f title="Phase 2 — Time series"     -f description="Multi-page dashboard with range charts. See docs/plans/02-time-series.md."
gh api "repos/$REPO/milestones" -f title="Phase 3 — Garmin metrics"  -f description="Activities, body battery, stress, HRV, sleep stages, body comp. See docs/plans/03-more-garmin.md."
gh api "repos/$REPO/milestones" -f title="Phase 4 — Manual inputs"   -f description="Training plan, training log, food log. See docs/plans/04-manual-inputs.md."
...
add_to_project() {
  gh project item-add "$PROJECT_NUMBER" --owner "@me" \
    --url "https://github.com/$REPO/issues/$1" >/dev/null
}
...
P1_T4=$(mk_issue "Add src/fitme/ingest.py — per-metric ingest functions and CLI" \
  "Idempotent INSERT OR REPLACE upserts, with --since / --from / --to / --date / --metrics flags." \
...
echo "Open the project and add custom fields (Phase, Type) + group Board view by Status."
